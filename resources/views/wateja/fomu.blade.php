@extends('layout')
@section('kichwa', isset($mteja) ? 'تعديل عميل' : 'إضافة عميل')

@section('maudhui')
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100">
            <h2 class="font-semibold text-slate-800">{{ isset($mteja) ? 'تعديل عميل: ' . $mteja->jina : 'إضافة عميل جديد' }}</h2>
        </div>
        ...
        <form method="POST"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <label class="relative cursor-pointer">
                        ...
                        <div class="peer-checked:border-blue-500 peer-checked:bg-blue-50 border-2 border-slate-200 rounded-xl p-4 text-center transition-all hover:border-slate-300">
                            ...
                            <p class="text-xs text-slate-500">دينار عراقي</p>
                        </div>
                    </label>
                    <label class="relative cursor-pointer">
                        ...
                        <div class="peer-checked:border-green-500 peer-checked:bg-green-50 border-2 border-slate-200 rounded-xl p-4 text-center transition-all hover:border-slate-300">
                            ...
                            <p class="text-xs text-slate-500">دولار أمريكي</p>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-1">
                <button type="submit"
                    ...
                    {{ isset($mteja) ? 'حفظ التعديلات' : 'إضافة العميل' }}
                </button>
