...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Mteja</label>
            <select name="mteja_id" id="sel-mteja"
                ...
                <option value="">— Mgeni (bila mteja) —</option>
                @foreach($wateja as $m)
                    <option value="{{ $m->id }}"
        ...
        {{-- Sarafu --}}
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Sarafu <span class="text-red-500">*</span></label>
            <div class="grid grid-cols-2 gap-2">
                <label class="cursor-pointer">
                    ...
                    <div class="peer-checked:border-blue-500 peer-checked:bg-blue-600 peer-checked:text-white border-2 border-slate-200 rounded-xl py-3 text-center transition-all font-bold text-sm hover:border-blue-300">
                        IQD
                        <div class="text-xs font-normal opacity-80">Dinari</div>
                    </div>
                </label>
                <label class="cursor-pointer">
                    ...
                    <div class="peer-checked:border-green-500 peer-checked:bg-green-600 peer-checked:text-white border-2 border-slate-200 rounded-xl py-3 text-center transition-all font-bold text-sm hover:border-green-300">
                        USD
                        <div class="text-xs font-normal opacity-80">Dola</div>
                    </div>
                </label>
            </div>
        </div>
        ...
        {{-- Tarehe --}}
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Tarehe <span class="text-red-500">*</span></label>
    ...
    {{-- Aina ya Malipo --}}
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <label class="block text-sm font-semibold text-slate-700 mb-3">Aina ya Malipo <span class="text-red-500">*</span></label>
        <div class="grid grid-cols-3 gap-3">
            <label class="cursor-pointer">
                ...
                <div class="peer-checked:border-green-500 peer-checked:bg-green-50 border-2 border-slate-200 rounded-xl p-4 text-center transition-all hover:border-slate-300">
                    ...
                    <p class="font-bold text-slate-800 text-sm">Taslimu</p>
                    <p class="text-xs text-slate-500">Pesa mkononi</p>
                </div>
            </label>
            <label class="cursor-pointer">
                ...
                <div class="peer-checked:border-orange-500 peer-checked:bg-orange-50 border-2 border-slate-200 rounded-xl p-4 text-center transition-all hover:border-slate-300">
                    <div class="text-2xl mb-1">📋</div>
                    <p class="font-bold text-slate-800 text-sm">Deni</p>
                    <p class="text-xs text-slate-500">Atalipa baadaye</p>
                </div>
            </label>
            <label class="cursor-pointer">
                ...
                <div class="peer-checked:border-purple-500 peer-checked:bg-purple-50 border-2 border-slate-200 rounded-xl p-4 text-center transition-all hover:border-slate-300">
                    <div class="text-2xl mb-1">📅</div>
                    <p class="font-bold text-slate-800 text-sm">Awamu</p>
                    <p class="text-xs text-slate-500">Malipo ya sehemu</p>
                </div>
            </label>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 class="font-semibold text-slate-800">Bidhaa Zinazouzwa</h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm" id="jedwali-bidhaa">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase w-2/5">Bidhaa</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase w-1/6">Idadi</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase w-1/4">Bei (kwa moja)</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase w-1/5">Jumla</th>
                ...
                <tbody id="safu-bidhaa">
                    <tr class="safu-bidhaa-mstari">
                        ...
                        <td class="px-4 py-3 text-center">
                            <button type="button" class="btn-futa-safu text-red-400 hover:text-red-600 p-1" title="Futa">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tfoot class="bg-slate-50">
                    <tr>
                        <td colspan="3" class="px-4 py-3 text-right font-semibold text-slate-700">JUMLA YOTE:</td>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5" id="sehemu-malipo">
        <h3 class="font-semibold text-slate-800 mb-4">Maelezo ya Malipo</h3>
        ...
        <div id="sehemu-taslimu">
            <div class="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <p class="text-green-800 text-sm font-medium">Malipo kamili yatalipwa sasa hivi.</p>
            </div>
        </div>
        ...
        <div id="sehemu-deni" class="hidden">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Kiasi Kilicholipwa Sasa</label>
                    ...
                    <p class="text-xs text-slate-400 mt-1">0 = deni lote, au ongeza malipo ya sehemu</p>
                </div>
                <div class="flex items-end">
                    <div class="w-full bg-orange-50 border border-orange-200 rounded-xl p-4">
                        <p class="text-xs text-orange-600 font-medium">Salio litakalodaiwa:</p>
        ...
        <div id="sehemu-awamu" class="hidden space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Malipo ya Kwanza (Amana)</label>
                    <input type="number" name="kiasi_ilipwa" id="kiasi-amana"
                           value="{{ old('kiasi_ilipwa', 0) }}" min="0" step="0.01"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Idadi ya Awamu <span class="text-red-500">*</span></label>
                    <input type="number" name="idadi_awamu" id="idadi-awamu"
                           value="{{ old('idadi_awamu', 6) }}" min="1" max="60"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Siku Baina ya Awamu</label>
                    <select name="siku_baina" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="7">Kila wiki (7 siku)</option>
                        <option value="14">Kila wiki 2 (14 siku)</option>
                        <option value="30" selected>Kila mwezi (30 siku)</option>
                        <option value="60">Kila miezi 2</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Tarehe ya Awamu ya Kwanza</label>
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-xl p-4">
                <p class="text-xs text-purple-600 font-medium mb-1">Kila awamu:</p>
                <p class="text-xl font-bold text-purple-700" id="kiasi-awamu-moja">—</p>
                <p class="text-xs text-purple-500 mt-1">Salio baada ya amana &divide; idadi ya awamu</p>
            </div>
        </div>
    ...
    {{-- Notes --}}
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <label class="block text-sm font-semibold text-slate-700 mb-2">Maelezo (hiari)</label>
    ...
    tr.innerHTML = `
        <td class="px-4 py-3">
            <select name="bidhaa[${index}][bidhaa_id]" required
                    class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bidhaa-sel">
                <option value="">— Chagua bidhaa —</option>${opts}
            </select>
        </td>
        <td class="px-4 py-3">
